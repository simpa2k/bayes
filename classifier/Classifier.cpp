...
Classifier::Classifier(shared_ptr<mt19937> engine) : engine(engine) {}
 ...
 * by its index.
 */
shared_ptr<mat> Classifier::computeThetaHidden(const umat& hiddenData) {
 ...
 * @return
 */
shared_ptr<mat> Classifier::computeThetaVisibleForNode(const umat& hiddenData, int hiddenStates, const umat& visibleData) {
 ...
 */
shared_ptr<vector<mat>> Classifier::computeThetaVisible(umat& hiddenData, umat& visibleData) {
 ...
 */
shared_ptr<mat> Classifier::replaceAllValues(umat& dataVisible, int thetaHidden, vector<mat>& thetaVisible) {
 ...
 * a state.
 */
shared_ptr<mat> Classifier::imputeHiddenNode(umat& dataVisible, mat& thetaHidden, vector<mat>& thetaVisible, bool generateNewData) {
 ...
 */
shared_ptr<mat> Classifier::learn(umat& dataHidden, umat& dataVisible, int learningIterations) {
